...
use Model\Entity\bentatecnologies_usuarios;
...
class UserRepository extends EntityRepository
{
        ...
        /*trabajo*/ 
        public function newUser ($c_username,$c_email,$c_nombre,$c_apellidos,$c_password,$n_idtipo_usuario) {
            //Si fuera añadir un user nuevo ----> $user = new bentastore_usuarios() y ya despues seria igual
            $user = new bentatecnologies_usuarios($c_username,$c_email,$c_nombre,$c_apellidos,$c_password,$n_idtipo_usuario);
            if($user) {
                //Hacerlo con todos los argumentos de entrada
                $user->setc_nickname($c_username);
                $user->setc_email($c_email);
                $user->setc_nombre($c_nombre);
                $user->setc_apellidos($c_apellidos);
                $user->setc_password($c_password);
                $user->setn_idtipo_usuario($n_idtipo_usuario);
                $user->setI_activo(true);
                $user->sett_conexiones(true);
        ...
        public function updateUser($c_username, $c_email, $c_nombre, $c_apellidos, $c_password) {
        $user = $this->findOneBy(['c_nickname' => $c_username]);

        if ($user) {
            // Set all the properties that need to be updated
            $user->setc_email($c_email);
            $user->setc_nombre($c_nombre);
            $user->setc_apellidos($c_apellidos);
            $user->setc_password($c_password);
        ...
        public function loginUser ($t_conexiones, $n_idusero) {
            $logeado = new bentatecnologies_usuarios($t_conexiones,$n_idusero);
            if($logeado) {
                //Hacerlo con todos los argumentos de entrada
       ...
       //Verifica q la sesion de ese usuario ya existe y no puedes registrarte ya que son exactamente los mismos datos
        ...
        {
            $userExists = $this->findOneBy(['c_nickname'=>$nickname, 'c_email'=>$email]);
                if($userExists){
                    return true;
                } else {
                    return false;
        ...
        public function getUsersOrderActives($filtroRol = '', $searchNickname = '')
        {
            $qb = $this->_em->createQueryBuilder();
            $qb->select('u')->from('\Model\Entity\bentatecnologies_usuarios', 'u')->innerJoin('u.n_idtipo_usuario', 'tu')->orderBy('u.c_nickname', 'ASC');

            if (!empty($filtroRol)) {
                $qb->where('tu.c_descripcion = :filtroRol')->setParameter('filtroRol', $filtroRol);
            }

            if (!empty($searchNickname)) {
                $qb->andWhere('u.c_nickname LIKE :searchNickname')->setParameter('searchNickname', '%'. $searchNickname. '%');
            }
          
            $query = $qb->getQuery();
            $actives = $query->getResult();
