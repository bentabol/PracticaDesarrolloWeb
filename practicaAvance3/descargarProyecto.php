...
if (isset($_POST['btDescargar'])) {
    ...
    /*if (isset($_POST['idProyecto']) && isset($_POST['idUsuario'])) {
        $descargas = $InformesRepository->getDescargasUser($_POST['idProyecto'], $_POST['idUsuario']);
        if ($InformesRepository->informeUserExist($_POST['idProyecto'], $_POST['idUsuario']) == true) {
            $descargas2 = $descargas->getT_descargas() + 1;              
            $InformesRepository->actualizarInformeDescargas($_POST['descargas2'], $_POST['idProyecto'], $_POST['idUsuario']); 

           
            */
            $proyectURL = $ProyectRepository->getUrlProyect($_POST['idProyecto']);
            if($proyectURL == NULL){
                $message = 'Lo sentimos, debe haber un problema al descargar el proyecto. Porfavor, intentelo de nuevo';
            }else {
                header("Content-disposition: attachment; filename=archivosProyectos/$proyectURL");
                header("Content-type: application/zip");
                readfile("$proyectURL");
                $message = 'Proyecto descargado correctamente. ';
                }
        /*
            ...
            //$InformesRepository->uploadInforme($_POST['fecha'],$_POST['descargas'],$_POST['idproyecto'],$_POST['idusuario']); 
            
                header("Content-disposition: attachment; filename=archivosProyectos/" . $ProyectRepository->getUrlProyect($_POST['idProyecto']));
                header("Content-type: application/zip");
                readfile("archivosProyectos/" . $ProyectRepository->getUrlProyect($_POST['idProyecto']));
                $message = 'Proyecto descargado correctamente. ';
                header("Location: misProyectos.php");
        }
        */
